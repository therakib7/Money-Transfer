<template>
    <div class="component">
        ...
        <div class="component-body">
            ...
            <!-- Modal Create-->
            <div v-can="'contact-taxonomy-create'" class="modal fade" id="createModal">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">{{ (formType) ? $t('new') : $t('update') }} {{ title() }}</h5>
                        ...
                        <form v-on:submit.prevent="saveForm()">
                            <div class="modal-body"> 
                                ...
                                <div class="form-row"> 
                                    ...
                                    <div class="col-12 mb-3" v-if="allTaxonomies.length">
                                        <label for="parent_id" class="o-s-l">{{ $t('parent') }} {{ title() }}</label> 
                            ...
                            <div class="modal-footer">
                                ...
                                <button class="btn btn-primary" type="submit">{{ (formType) ? $t('create') : $t('update') }}</button>
                            </div><!--  /.modal-footer -->
                        </form>
                    </div><!--  /.modal-content -->
                </div><!--  /.modal-dialog -->
                        ...
                        <form v-on:submit.prevent="dataSearch()">
                            <div class="modal-body"> 
                                ...
                                <div class="form-row"> 
                                    ...
                                    <div class="col-12 mb-3"> 
                                        <date-time-picker v-model="taxonomySearch.date_from" value-format="yyyy-LL-dd HH:mm:ss" format="dd-LLLL-yyyy hh:mm a" :today-button="true" :close-button="true" :hour-time="12"><template v-slot:choose-date>{{ $t('date_from') }}</template></date-time-picker>
                                    </div><!--  /.col-12 mb-3 --> 

                                    <div class="col-12 mb-3"> 
                                        <date-time-picker v-model="taxonomySearch.date_to" value-format="yyyy-LL-dd HH:mm:ss" format="dd-LLLL-yyyy hh:mm a" :today-button="true" :close-button="true" :hour-time="12"><template v-slot:choose-date>{{ $t('date_to') }}</template></date-time-picker>
                                    </div><!--  /.col-12 mb-3 --> 
            ...
            <div class="table-actions">
                <button v-can="'contact-taxonomy-create'" type="button" @click="newForm()" class="btn btn-sm bg-success" data-toggle="modal" data-target="#createModal">
                    <i class="icon-plus-squared"></i>{{ title() }}
                </button> 

                <button v-can="'contact-taxonomy-delete'" v-if="selected.length" class="btn btn-sm btn-danger " v-on:click="deleteSelected()"><i class="icon-trash-empty"></i> {{ $t("d_s") }}</button>
                            ...
                            <td>
                                <div class="dropdown action-items">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="actionItemButtons">
                                        <a v-can="'contact-taxonomy-edit'" href="#" class="dropdown-item" v-on:click.prevent="editForm(taxonomy)" data-toggle="modal" data-target="#createModal">
                                           {{ $t('edit') }} 
                                        </a>
                                        <a v-can="'contact-taxonomy-delete'" href="#" class="dropdown-item" v-on:click.prevent="deleteEntry(taxonomy.id, index)">
                                            {{ $t('delete') }}
                                        </a>
                                    </div>
                                </div> 
...
export default {
    ...
    methods: {  
        ...
        getLists( page = 1, click = false ) {
            ...
            axios.get('/api/v1/ct/taxonomies/'+ app.tax+'?'+ queryString)
                .then(function (resp) {
                    app.preloader = false;
        ...
        saveForm() { 
            ...
            if ( this.formType ) {
                axios.post('/api/v1/ct/taxonomies', newTaxonomy)
                    .then( resp => {  
                        this.taxonomy = this.taxonomyConst;
                ...
                axios.patch('/api/v1/ct/taxonomies/' + newTaxonomy.id, newTaxonomy)
                    .then( resp => { 
        ...
        deleteEntry(id, index) {
            ...
            if (confirm(app.$store.state.confirmDel)) {
                axios.delete('/api/v1/ct/taxonomies/' + id)
                    .then(function (resp) {
        ...
        deleteSelected() { 
            ...
            if (confirm(app.$store.state.confirmDel)) {
                ...
                axios.delete('/api/v1/ct/taxonomies/' + selectedItem)
                    .then(function (resp) {
