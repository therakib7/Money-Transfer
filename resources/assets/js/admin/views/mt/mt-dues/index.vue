		...
		<div class="component-body"> 
				...
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title">{{ $t('n_d_a') }}</h5>
								...
								<div class="form-row">  
									...
									<div class="col-md-12">  
										<div class="input-group outline-group mobile-number"> 
											...
											<label for="number">
											{{ ( due.search_name ) ?  $t('c_n') : $t('m_n') }} {{ ( due.name ) ? `(${due.name})` : '&nbsp;'}}  
											</label>
											...
											<div class="input-group-append" v-if="!due.number">
												<span class="input-group-text cursor-pointer font-weight-bold" :title="$t('c_l')" v-on:click.prevent="loadContact(null, null)" data-toggle="modal" data-target="#contactModal"><i class="icon-address-book"></i></span>  
											</div>
											<div class="input-group-append">
												<span class="input-group-text cursor-pointer font-weight-bold" :title="( due.add_name ) ?  $t('remove_name') : $t('a_n_n')" @click="due.add_name = !due.add_name"><i class="icon-user-plus"></i></span>  
											</div>
										</div> 
									</div><!--  /.col-md-12 mb-3 -->  
									...
									<div class="col-md-12 mb-3">
										<div class="form-row"> 
											...
											<div class="col-sm-7 col-xs-12 mb-16">
												...
												<multiselect :allow-empty="false" deselect-label="" select-label="" v-model="due.provider_id" :options="providers" :preserve-search="true" label="name" track-by="name" :placeholder="$t('s_o')" :preselect-first="false" @input="providerChanged()">
											...
											<div class="col-sm-5 col-xs-12">
												...
												<select class="form-control" id="service_id" v-model="due.service_id">  
													<option v-for="service in due.services" :value="service">{{service.name}}</option>
												</select>  
			...
			<div v-can="'due-amount-create'" class="modal fade" id="createPaidModal">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title">{{ $t('new_paid_and_due') }}</h5>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							  <span aria-hidden="true">&times;</span>
							</button>
						</div>
						...
						<form v-on:submit.prevent="savePaidForm()">
							<div class="modal-body"> 
								...
								<div class="form-row">  
									...
									<div class="col-md-12 mb-3">
										<input placeholder=" " type="number" class="form-control outline" id="paid-due-amount" v-model="paid.amount">
										<label for="paid-due-amount">{{ $t('tk') }}</label>
									</div><!--  /.col-md-12 mb-3 --> 
									...
									<div class="col-md-12 mb-3">
										<!-- <date-time-picker :placeholder="$t('date_time')" v-model="paid.date_time"></date-time-picker> -->
										<date-time-picker v-model="paid.date_time" value-format="yyyy-LL-dd HH:mm:ss" format="dd-LLLL-yyyy hh:mm a" :today-button="true" :close-button="true" :hour-time="12"><template v-slot:choose-date>{{ $t('choose_date') }}</template></date-time-picker>
									</div><!--  /.col-md-12 mb-3 -->  
							...
							<div class="row">
								<div class="col-md-3">
									...
									<div class="form-row d-none d-md-block">  
										...
										<div class="col-12 mb-3"> 
											<label class="o-s-l">{{ $t('operator') }}</label>
											<multiselect deselect-label="" select-label="" v-model="history_filter.provider_id" :options="providers" :preserve-search="true" label="name" :placeholder="$t('s_o')" track-by="name" :preselect-first="false">
												<template slot="singleLabel" slot-scope="props">
													<template v-if="props.option.logo_small_src">
														<img class="option__image" :src="'/uploads/imgs/'+props.option.logo_small_src.resized_name" :alt="props.option.name">
													</template> 
										...
										<div class="col-12 mb-3" v-if="history_filter.provider_id"> 
											<label for="search-service_id" class="o-s-l">{{ $t('o_t') }}</label>
					...
					<div class="modal-content">
						...
						<div class="modal-body">  
							<div class="form-row"> 
								...
								<div class="col-12 mb-3"> 
									<label class="o-s-l">{{ $t('operator') }}</label>
									<multiselect deselect-label="" select-label="" v-model="history_filter.provider_id" :options="providers" :preserve-search="true" label="name" track-by="name" :placeholder="$t('s_o')" :preselect-first="false">
										<template slot="singleLabel" slot-scope="props">
											<template v-if="props.option.logo_small_src">
												<img class="option__image" :src="'/uploads/imgs/'+props.option.logo_small_src.resized_name" :alt="props.option.name">
											</template> 
								...
								<div class="col-12 mb-3"> 
									<div class="form-row">   
										<div class="col-md-6 mb-3">  
											<input placeholder=" " type="text" class="form-control outline" id="filter-amount" v-model="history_filter.amount_from">
                                        <label for="history-amount-from">{{ $t("amount_from") }}</label>
										</div><!--  /.col-md-6 -->
										<div class="col-md-6">
											<input placeholder=" " type="text" class="form-control outline" id="filter-amount" v-model="history_filter.amount_to">
                                        	<label for="history-amount-to">{{ $t("amount_to") }}</label> 
										</div><!--  /.col-md-6 -->
									</div><!--  /.form-row -->
								</div><!--  /.col-12 mb-3 -->  
						...
						<tr v-for="due, index in dues.data"> 
							...
							<td>
								<div class="dropdown action-items">
								  ...
								  <div class="dropdown-menu ml-23" aria-labelledby="actionItemButtons"> 
...
export default {
    ...
    methods: { 
		...
		numberChange(key, searchName = false ) { 
			...
			if ( query.length >= 3 ) {  
				...
				if ( operator.length ) {
					this.due.provider_id = operator[0];  
					if ( operator[0].services ) {
						this.due.service_id = operator[0].services[0];  
						this.due.services = operator[0].services; 
					} 
				} 
			} else {
	...
	i18n: { // `i18n` option, setup locale info for component
		messages: {
			en: { 
				m_t_due_list: 'Money Transfer Due Lists', 
				...
				//Modal Payment History
				c_p: 'Choose Package', 
				new_paid_and_due: 'New Paid/Due', 
				due_amount: 'Due Amount', 
				date_time: 'Date Time', 
				paid_due: 'Paid Due', 
				history_filter: 'History Filter', 
				//modal-content 
				n_d_a: 'New Due Amount',
				m_n: 'Mobile Number',
				c_n: 'Contact Name',
				o_t: 'Operator Type',
			...
			bn: { 
				m_t_due_list: 'টাকা ট্রান্সফার বাকি তালিকা',  
				due_amount: 'বাকি টাকা', 
				paid: 'পরিশোধ', 
				due: 'বাকি', 
				paid_due: 'পরিশোধ / বাকি', 
				history: 'ইতিহাস', 
				//Modal Payment History
				c_p: 'প্যাকেজ পছন্দ করুন',
				new_paid_and_due: 'নতুন পরিশোধ/বাকি',
				due_amount: 'বাকি টাকার পরিমান',
				date_time: 'তরিখের সময়',
				paid_due: 'বাকি পরিশোধ', 
				history_filter: 'ইতিহাস ফিল্টার',
				//modal-content 
				n_d_a: 'নতুন বাকির পরিমান',
				m_n: 'মোবাইল নাম্বার',
				c_n: 'ফোনবুকের নাম',
				o_t: 'অপারেটর ধরন',
				choose_date: 'তারিখ পছন্দ করুন',
				//contact list
				c_l: 'ফোনবুক তালিকা', 
				//search-content 
				paid: 'পরিশোধ', 
