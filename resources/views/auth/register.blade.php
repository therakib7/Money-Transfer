...
<body>
    <div class="main-content">
        <div class="card card-reg">
            <div class="card-header">{{ __('Create New Account') }}
            </div>
            <form method="POST" action="{{ route('register') }}" aria-label="{{ __('Register') }}">
                @csrf

                <div class="form-group"> 
                    <input type="hidden" name="ref_id" value="{{ app('request')->input('ref_id') }}" >
                    
                    <div class="outline-group">
                        <input id="name" type="text" placeholder="name" class="form-field outline{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name') }}" required autofocus>
                    ...
                    <div class="outline-group">
                        <input id="email" type="text"  placeholder="email" class="form-field outline{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required>
                        <label for="email" class="col-md-4 col-form-label text-md-right">{{ __('Mobile Or Email') }}</label>

                        @if ($errors->has('email'))
                            <span class="invalid-feedback" role="alert">
                                <strong>Mobile number or Email address already exits.</strong>
                            </span>
                        @endif
                    </div>  
                    ...
                    <div class="checkbox">
                        <label>
                            <input type="checkbox" name="terms_and_conditions" {{ old('terms_and_conditions') ? 'checked' : '' }}> {{ __('I have read and agreed to the') }} <a href="{{ route('terms_n_conditions') }}">Terms and conditions</a>
                        </label>
                    </div>
